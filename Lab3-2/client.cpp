...
void sendFile()
{
	...
	leftWin = rightWin = 0;
	HANDLE recvThread = CreateThread(NULL, 0, recvThreadFunction, 0, 0, NULL);					   // 接收ACK线程
	HANDLE reTransmitFileThread = CreateThread(NULL, 0, reTransmitFileThreadFunction, 0, 0, NULL); // 重传线程
	while (1)
	{
		// 窗口未满，未发送完，可以发送
		if (rightWin - leftWin < MAXWIN & rightWin < rounds)
		{
			...
			if (leftWin == rightWin)
			{
				timer.setStart(times[rightWin]); // 如果窗口为空的时候发的包，设置计时器为相应时间
			}
			rightWin++; // 发送后，右窗口右移
	...
	TerminateThread(reTransmitFileThread, 0);
	CloseHandle(recvThread);
	CloseHandle(reTransmitFileThread);
	timer.stopTimer();
	timer.stop();
}
